#+ Database creation and seed-load script for the Northwind sample database.
#+
...
#+ Character type for columns wider than 255 chars. Informix VARCHAR is
#+ capped at 255 bytes, so use LVARCHAR there; VARCHAR elsewhere.
FUNCTION sql_longvarchar(n INTEGER) RETURNS STRING
    IF m_drv == "ifx" THEN
        RETURN SFMT("LVARCHAR(%1)", n)
    END IF
...
FUNCTION db_create_tables()
    ...
    EXECUTE IMMEDIATE SFMT("CREATE TABLE customerdemographics (customertypeid CHAR(20) NOT NULL, customerdesc %1)",
...
#+ Create one auto-increment sequence per primary-key table.
#+
#+ The seed-data INSERTs above supply explicit IDs, so each sequence must
#+ start just past the highest seeded id; otherwise the first application
#+ insert would reuse id=1 and collide with the seed rows.
#+
...
FUNCTION db_create_sequences()
    ...
    CALL create_seq("orders_orderid_seq",        "orderid",    "orders")
    CALL create_seq("products_productid_seq",    "productid",  "products")
    CALL create_seq("region_regionid_seq",       "regionid",   "region")
    CALL create_seq("shippers_shipperid_seq",    "shipperid",  "shippers")
    CALL create_seq("suppliers_supplierid_seq",  "supplierid", "suppliers")
    CALL create_seq("usstates_stateid_seq",      "stateid",    "usstates")

END FUNCTION #db_create_sequences

#+ Create one sequence positioned just past the highest existing id.
#+   seqname - sequence name to create
#+   col     - primary-key column to scan
#+   tab     - table holding the seed data
FUNCTION create_seq(seqname STRING, col STRING, tab STRING)
    DEFINE nextid BIGINT
    DEFINE stmt STRING

    LET stmt = SFMT("SELECT COALESCE(MAX(%1),0)+1 FROM %2", col, tab)
    PREPARE p_seqmax FROM stmt
    EXECUTE p_seqmax INTO nextid
    FREE p_seqmax

    LET stmt = SFMT("CREATE SEQUENCE %1 START WITH %2", seqname, nextid)
    EXECUTE IMMEDIATE stmt
END FUNCTION #create_seq
